...
#=
"""
This type is identical to the JonesMatrix type
except that it is more compatible with the
StructsOfArrays package.
"""
immutable SplayedJonesMatrix
    xxr::Float32
    xxi::Float32
    xyr::Float32
    xyi::Float32
    ...
    yyr::Float32
    yyi::Float32
    @inline function SplayedJonesMatrix(xxr,xxi,xyr,xyi,
                                        yxr,yxi,yyr,yyi)
        new(xxr,xxi,xyr,xyi,yxr,yxi,yyr,yyi)
    end
end

function SplayedJonesMatrix(xx::Complex64,xy::Complex64,
                            yx::Complex64,yy::Complex64)
    SplayedJonesMatrix(real(xx),imag(xx),
                       real(xy),imag(xy),
                       real(yx),imag(yx),
                       real(yy),imag(yy))
end

function SplayedJonesMatrix(J::JonesMatrix)
    SplayedJonesMatrix(J.xx,J.xy,J.yx,J.yy)
end

function JonesMatrix(J::SplayedJonesMatrix)
    JonesMatrix(complex(J.xxr,J.xxi),
                complex(J.xyr,J.xyi),
                complex(J.yxr,J.yxi),
                complex(J.yyr,J.yyi))
end
